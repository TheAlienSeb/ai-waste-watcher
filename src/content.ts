...
let livePreviewElement: HTMLElement | null = null;
...
function injectLivePreview() {
  ...
  const closeButton = document.getElementById('ai-waste-close');
  if (closeButton) {
...
function calculateImpact(responseTokens: number, inputTokens: number, model: string): {
  waterUsage: number;
  carbonEmissions: number;
  energyConsumption: number;
  cost: number;
  ...
  // Get model factor
  const modelFactor = MODEL_FACTORS[model as keyof typeof MODEL_FACTORS] || MODEL_FACTORS.default;
  const modelParams = MODEL_PARAMETERS[model as keyof typeof MODEL_PARAMETERS] || MODEL_PARAMETERS.default;
...
function processPrompt(text: string, model: string) {
